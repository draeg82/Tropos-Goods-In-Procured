...
{
    public partial class OrderLineReceipts : TroposUI.Common.UI.TroposPage
    {
        ...
        protected void lnkRejectNoteNo_Command(object sender, CommandEventArgs e)
        {
            if (sender is LinkButton)
            {
                string rejectNoteNo = e.CommandArgument.ToString();
                LinkButton lb = (LinkButton)sender;
                Dictionary<string, string> Fields = new Dictionary<string, string>();
                Fields.Add("RejectNote", rejectNoteNo);

                int width = (int)(Convert.ToDouble(screenWidth.Value) * 0.75);
                int height = (int)(Convert.ToDouble(screenHeight.Value) * 0.5);

                PopupPage pp = new PopupPage(updDummy, "RejectNoteInformation", "Reject+Note+Information", false, 50, 50, width, height, Fields);
                updDummy.Update();
