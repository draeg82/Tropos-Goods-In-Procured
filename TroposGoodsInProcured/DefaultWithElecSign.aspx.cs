...
{
    public partial class DefaultWithElecSign : TroposUI.Common.UI.TroposPage
    {
        ...
        private void Transaction_DataValid(object sender, DataValidEventArgs e)
        {
            // Show the data valid dialog
            string LoadedScript = "ModalLoaded();";
            ScriptManager.RegisterStartupScript(pnlPopup_DataValid, GetType(), "ModalLoaded", LoadedScript, true);
            foreach (KeyValuePair<string, string> Davl in e.Reasons)
            {
                System.Web.UI.WebControls.ListItem Item = new System.Web.UI.WebControls.ListItem(Davl.Value, Davl.Key);
                lstDvReason.Items.Add(Item);
            }
            pnlPopup_DataValid.Show();
        }

        protected void btnYes_Click(object sender, EventArgs e)
        {
            ProcessDV(TroposConfirmationResponse.Yes);
        }
        protected void btnNo_Click(object sender, EventArgs e)
        {
            ProcessDV(TroposConfirmationResponse.No);
        }
        protected void btnDefer_Click(object sender, EventArgs e)
        {
            ProcessDV(TroposConfirmationResponse.Defer);
        }
        protected void btnBackground_Click(object sender, EventArgs e)
        {
            ProcessDV(TroposConfirmationResponse.Background);
        }
        private void ProcessDV(TroposConfirmationResponse response)
        {
            if (response == TroposConfirmationResponse.No)
                return;             // Nothing to do.

            DVResponse = response;                                          // Save the user response

            throw new NotImplementedException();
        }
        TroposConfirmationResponse DVResponse;
        /// <summary>
        /// Called when the transaction execution goes to data-valid
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void Transaction_DataValidAfterSignature(object sender, DataValidEventArgs e)
        {
            e.Response = DVResponse;                            // Use the saved response
            DVResponse = TroposConfirmationResponse.No;         // Clear the saved response (to prevent a loop if the signature is invalid)
            if (e.ElectronicSignatureRequired)                  // If a signature is required, set up the data
            {
                e.Signature.Identity = txtDvIdentity.Text;
                e.Signature.Password = txtDvPassword.Text;
                e.Signature.Comment = txtDvComment.Text;
                e.Signature.Reason = lstDvReason.SelectedItem?.Value;
            }
        }
